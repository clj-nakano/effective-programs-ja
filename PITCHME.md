...
- LWW Lightweight language Workshop ... MITでの軽量原語の勉強会
    - たまたま出会ったコンピュータ言語リサーチャーが、データベースに「ハマった」同僚についてバカにして話しているのを聞いて、ピザをを喉につまらせるほど驚いた
...
- 現実の不整合にたいおうしていくことがこの種のプログラムでは非常に重要
    - スケジュール管理であれば、同じ曲、同じアーティストの曲を視聴者が聞いている時間軸の間に繰り返し流さないことが目的で、せっかくアルゴリズムで最適化している
...
- Effectiveを、Clojureのように企業向けのアプリケーションに使われる言語の目的として取り戻したい
...
- Clojureがライブラリを記述するのに適した言語にしようとはしたが、ライブラリのエコシステムについては触れていない。昨年の"Library implies"で述べたようにこれは、将来解決すべき問題と考えている。

--- 
## Clojure Design Objectives

- 18年C++とかでプログラムしてきて、もううんざり
- （会場で挙手してもらって）Clojureはビギナー向けの言語が、疲れ果てて不機嫌になった年寄りプログラマのためのものかな？
...
- だが、パフォーマンスとディプロイについて答えを要していなければ親しい友だちにも使ってもらえない。
...
- (PLOPについては別のキーノートで詳説されている。とりあえず手続き型プログラミングと理解してください）
...
- BagwellのHash Array Map Trie はパーシステンス構造ではなかったが、自分が改良し、はじめて満足のいくパフォーマンスが得られた。
- 関数型プログラミングの慣用法を支える多くの純粋関数群
- 一番の障壁はカッコではなく、関数型の考え方。それをささえるためにたくさんのライブラリを書いた。
- 関数プログラミングのコミュニティの大部分は関数型言語=強い型付き静的言語、と思っているが、型は関数型言語に必須な要件ではなく、関数型のメリットは80:20どころではなく、99%型を使わずに得ることができる。

---
## Information

- 静的言語でもっともイライラさせるポイント 情報の扱いたが下手すぎる
...
- Clojureは動的タイプ。情報を送る人、使う人、その間で情報を伝達する人はどんなデータタイプが送られてきているのか、考える必要がない。
...
- 情報の表現の仕方は、その言語特有のローカルールで表現することになる。
...
- Listは慣用的に関数的に使われているが、他の構造はそうではない。
- Listの実装がダメダメ
